import {z} from "zod";
const callTimeContraint = z.string().min(7,{message: "la descripcion del tiempo de llamada de no puede tener menos de 7 caracteres"}).max(120, {message:"la descripcion del tiempo de llamada no puede exceder los 120 caracteres"});
...
const locationContraint = z.string().min(10, {
...
export const summaryValidator = z.object({
    ...
    }).strict("los unicos campos disponibles son: callTime, incident, requestor, notes,location y phone")
});
...
export const summaryParamsValidator = z.object({
   params:z.object({
        ...
        summaryId:z.string().min(1,{message:"el numero no puede ser negativo"}).regex(new RegExp("^[1-9]\d*$"),{message:"el numero no puede incluir letras,caracteres especiales ni tampoco puede incluir numeros negativos"})
...
export const summaryQueryValidator = z.object({
    query:z.object({
        dispatcher:z.string().min(6,{message: "el nombre del emisario no puede tener menos de 6 caracteres"}).max(60, {message:"el nombre del emisario no puede exceder los 60 caracteres"}).optional(),
       ...
       }).strict("los unicos parametros query disponibles son: dispatcher, limit y starting_after") 
});


export const summaryUpdateValidator = z.object({
    body:z.object({
        id:z.number().nonnegative({message:"los ids negativos no son validos"}),
        callTime:callTimeContraint,
        incident:incidentContraint,
        requestor:requestorContraint,
        notes:notesContraint,
        location:locationContraint,
        phone:phoneContraint
    }).strict("los unicos campos disponibles son: id, callTime, incident, requestor, notes, location y phone")
});